...
const add = async(req, res) => {
    ...
    if (!req.body.warehouse_id || !req.body.item_name || !req.body.description
        || !req.body.category || !req.body.status || !req.body.quantity) {
        return res.status(400).json({
            message: "Please provide necessary details for the inventory item in the request",
        });
    }else if (isNaN(req.body.quantity)) {    //Checks if quantity is a valid number. Returns false if not a number
        return res.status(400).json({
    ...
    const existingWarehouse = await knex("warehouses").where({id: req.body.warehouse_id})
    ...
    try {
        ...
        const createdInventoryItem = await knex("inventories").where({id: newInventoryId})//Retrieves the newly created item to return
        .select(
            "id",
            "warehouse_id",
            "item_name",
